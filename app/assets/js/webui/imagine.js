(() => {
  ...
  async function loadFilterDefaults() {
    try {
      const res = await fetch(`${FN_BASE}/imagine/config`, { cache: 'no-store' });
      if (!res.ok) return;
  ...
  async function createImagineTask(prompt, ratio, authHeader, nsfwEnabled) {
    const res = await fetch(`${FN_BASE}/imagine/start`, {
      method: 'POST',
      headers: {
  ...
  async function stopImagineTasks(taskIds, authHeader) {
    ...
    try {
      await fetch(`${FN_BASE}/imagine/stop`, {
        method: 'POST',
        headers: {
          ...buildAuthHeaders(authHeader),
          'Content-Type': 'application/json'
  ...
  function buildSseUrl(taskId, index, rawPublicKey) {
    ...
    const base = `${httpProtocol}://${window.location.host}${FN_BASE}/imagine/sse`;
  ...
  async function startConnection() {
    ...
    for (let i = 0; i < taskIds.length; i++) {
      ...
      const wsUrl = `${protocol}://${window.location.host}${FN_BASE}/imagine/ws?${params.toString()}`;
